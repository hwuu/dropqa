...
- [5. 核心算法设计](#5-核心算法设计)
  ...
  - [5.4 版本管理](#54-版本管理)
  - [5.5 结构规范化实现](#55-结构规范化实现)
    - [5.5.1 模块结构](#551-模块结构)
    - [5.5.2 核心数据结构](#552-核心数据结构)
    - [5.5.3 规范化器接口](#553-规范化器接口)
    - [5.5.4 段落切分器](#554-段落切分器)
    - [5.5.5 Section 拆分器](#555-section-拆分器)
    - [5.5.6 标题增强器](#556-标题增强器)
    - [5.5.7 主规范化器](#557-主规范化器)
    - [5.5.8 配置项](#558-配置项)
...
- [7. 接口设计](#7-接口设计)
  ...
  - [7.2 前端页面](#72-前端页面)
    ...
    - [7.2.2 项目结构](#722-项目结构)
    - [7.2.3 核心组件](#723-核心组件)
    - [7.2.4 组合式函数](#724-组合式函数)
    - [7.2.5 构建与部署](#725-构建与部署)
- [8. 测试方案](#8-测试方案)
  - [8.1 单元测试](#81-单元测试)
  - [8.2 集成测试](#82-集成测试)
  - [8.3 测试数据](#83-测试数据)
...
本节描述文档结构规范化模块的实际代码实现。

#### 5.5.1 模块结构

```
dropqa/indexer/normalizer/
├── __init__.py           # 模块导出
├── base.py               # 基类和数据结构
├── paragraph_splitter.py # 长段落切分
├── section_splitter.py   # 长 Section 拆分
├── title_enricher.py     # 无意义标题增强
└── normalizer.py         # DocumentNormalizer 主类
```

#### 5.5.2 核心数据结构

规范化模块复用解析模块的 `ParsedNode` 数据结构：
...
`ParagraphSplitter` 负责切分过长的段落：

```python
class ParagraphSplitter(BaseNormalizer):
    def __init__(
        self,
        max_length: int = 800,
        target_size: int = 400,
...
`SectionSplitter` 负责拆分过长且无子节点的 Section：

```python
class SectionSplitter(BaseNormalizer):
    def __init__(
        self,
        max_length: int = 3000,
...
**拆分流程**：
1. 检查 section 是否需要拆分（无子节点且内容超过 `max_length`）
2. 收集所有段落内容
3. 使用语义切分将内容分组
4. 使用 LLM 为每个分组生成标题
5. 创建新的子 section 节点
...
`TitleEnricher` 负责增强无意义的标题：

```python
class TitleEnricher(BaseNormalizer):
    # 无意义标题的正则模式
...
2. 收集节点内容预览（前 500 字符）
3. 使用 LLM 生成有意义的标题
4. 保留原编号，追加生成的标题（如 "一、项目背景"）

#### 5.5.7 主规范化器

`DocumentNormalizer` 串联所有规范化器：
...
class DocumentNormalizer:
    def __init__(
        ...
        # 按顺序添加规范化器
        self._normalizers.append(ParagraphSplitter(...))
        self._normalizers.append(SectionSplitter(...))

        if config.enrich_meaningless_titles and llm_func:
            self._normalizers.append(TitleEnricher(...))

    async def normalize(self, root: ParsedNode) -> ParsedNode:
        """依次应用所有规范化器"""
        result = root
...
normalization:
  ...
  paragraph_target_size: 400
  max_section_length: 3000
  enrich_meaningless_titles: true
```

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| enabled | false | 是否启用规范化 |
| max_paragraph_length | 800 | 段落最大长度 |
| paragraph_target_size | 400 | 段落目标长度 |
| max_section_length | 3000 | Section 最大长度 |
...
```
dropqa/server/frontend/
├── src/
│   ├── components/           # Vue 组件
│   │   ├── SearchBox.vue     # 搜索输入框
│   │   ├── AnswerCard.vue    # 答案卡片（含 Markdown 渲染）
│   │   ├── SourceList.vue    # 来源引用列表
│   │   ├── CopyButton.vue    # 复制按钮
│   │   ├── ReasoningTrace.vue # 推理过程展示
...
- 代码块语法高亮
- 回答模式标签（simple/agentic）
- 集成 SourceList 和 ReasoningTrace
...
export function useQA() {
  ...
  return { question, answer, loading, error, ask }
}
```
...
export function useHistory() {
  ...
  function load() { /* 从 LocalStorage 加载 */ }
  function save(item: HistoryItem) { /* 保存到 LocalStorage */ }
  function clear() { /* 清空历史 */ }

  return { history, load, save, clear }
}
```
...
```bash
# 开发模式
cd dropqa/server/frontend
npm install
npm run dev

# 生产构建
npm run build
# 输出到 dropqa/server/static/dist/
```

后端自动挂载 Vue 构建产物：
- `GET /` → 返回 `static/dist/index.html`
- `GET /assets/*` → 返回静态资源文件
...
**7. 结构规范化** ✅ 已完成（2024-12-15）
- 长段落语义切分（ParagraphSplitter）
- 长 Section 自动拆分（SectionSplitter）
- 无意义标题增强（TitleEnricher）
- 模块化设计，支持 LLM/Embedding 函数注入
- 单元测试覆盖（21 个新测试用例）
...
**12. 完整前端** ✅ 已完成（2024-12-15）
- Vue 3 + Vite + TailwindCSS 项目搭建
- 问答界面组件（SearchBox、AnswerCard、SourceList）
- 推理过程展示组件（ReasoningTrace）
